        ...
        switch (response) {
            case JsonResponse.JsonValue<T> jsonResponse -> {
                try {
                    ...
                    else if (jsonResponse.statusCode() == HttpStatus.SERVICE_UNAVAILABLE.code()) {
            ...
            case JsonResponse.Exception<T> exceptionResponse -> {
                if (exceptionResponse.statusCode() == HttpStatus.SERVICE_UNAVAILABLE.code()) {
                    onFailure(new ServiceUnavailableException(uri));
                }
                else {
            ...
            case JsonResponse.NonJsonBytes<T> nonJsonResponse -> {
                if (nonJsonResponse.statusCode() == HttpStatus.SERVICE_UNAVAILABLE.code()) {
                    onFailure(new ServiceUnavailableException(uri));
                }
                else {
                    callback.fatal(new TrinoException(REMOTE_TASK_ERROR, format("Expected response code from %s to be %s, but was %s%n%s",
                            uri,
                            HttpStatus.OK.code(),
            ...
            case null -> callback.fatal(new TrinoException(REMOTE_TASK_ERROR, format("Unexpected response from %s", uri)));
        }
    }
